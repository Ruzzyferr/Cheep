...
# preflight → prebuild → bundleRelease → signature check → cheep-<version>-vc<code>.aab
```

The **upload keystore lives outside the repo** (`~/CheepKeys`, overridable with
`CHEEP_KEYSTORE_DIR`); `plugins/withReleaseSigning.js` regenerates
`android/key.properties` from it on every prebuild. It used to live under
`android/app/` — the one directory `prebuild --clean` deletes — and a single
`--clean` destroyed it. A `PreToolUse` hook (`.claude/hooks/guard-destructive.mjs`)
now refuses that class of command outright.

Because a release can silently be signed with the **debug** key and only fail at
upload time, the script verifies the artifact's certificate against the vault and
exits non-zero on mismatch.

Full step-by-step (signing vault, version bumps, Play Console, iOS, push
